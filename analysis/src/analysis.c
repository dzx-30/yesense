...
unsigned char check_data_len_by_id(unsigned char id, unsigned char len, unsigned char *data)
{
	...
	case EULER_ID:
	{
		if (EULER_DATA_LEN == len)
		{
			...
			if (0 == angle_start_flag)
			{
				yaw_start = g_output_info.yaw;
				pitch_start = g_output_info.pitch;
				roll_start = g_output_info.roll;
				angle_start_flag = 1;
			}
			g_output_info.yaw -= yaw_start;
			g_output_info.roll -= roll_start;
			g_output_info.pitch -= pitch_start;
		}
		else
			ret = (unsigned char)0x00;
	}
